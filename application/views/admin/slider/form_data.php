        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <a href="<?= base_url('Admin/Slider') ?>" class="btn btn-warning"><i class="fas fa-backward"></i> Kembali</a>
                            <br><br>
                            <form action="<?= base_url('Admin/Slider/process') ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="id_slider" value="<?= $row->id_slider; ?>">
                                <div class="form-group">
                                    <label for="">Judul</label>
                                    <input placeholder="Judul" type="text" name="judul" class="form-control  <?= form_error('judul') != null ? 'border border-danger' : '' ?>" value="<?= $row->judul != null ? $row->judul : set_value('judul') ?>">
                                    <?= form_error('judul') ?>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    if ($row->nama_file != null) {
                                        echo '<img class="w-25" src="' . base_url('public/image/slider/' . $row->nama_file) . '"></img>';
                                    }
                                    ?>
                                    <?= form_error('c') ?>
                                </div>
